...
index <- grep ("^loc\\_", names (dat))
index_per <- grep ("^loc\\_per", names (dat))
index <- index [which (!index %in% index_per)]
loc_mat <- as.matrix (dat [, index])
dat$loc <- rowSums (loc_mat, na.rm = TRUE)
