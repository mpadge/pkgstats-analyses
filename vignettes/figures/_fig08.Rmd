...
dat_one_year <- function (xraw, year = 2015) {
    ...
    x$n_fns_r [is.na (x$n_fns_r)] <- 0L
    x$n_fns_src [is.na (x$n_fns_src)] <- 0L
    edges_rel <- x$n_edges / (x$n_fns_r + x$n_fns_src)
    edges_rel_r <- x$n_edges / x$n_fns_r
    edges_rel_src <- x$n_edges / x$n_fns_src
    edges_rel <- edges_rel [which (is.finite (edges_rel) & edges_rel > 0)]
    edges_rel_r <- edges_rel_r [which (is.finite (edges_rel_r) & edges_rel_r > 0)]
    edges_rel_src <- edges_rel_src [which (is.finite (edges_rel_src) & edges_rel_src > 0)]

    c (year = year,
       n_edges = logmean (x$n_edges, na.rm = TRUE),
       n_edges_r = logmean (x$n_edges_r, na.rm = TRUE),
       n_edges_src = logmean (x$n_edges_src, na.rm = TRUE),
       n_edges_rel = logmean (edges_rel),
       n_edges_rel_r = logmean (edges_rel_r),
       n_edges_rel_src = logmean (edges_rel_src),
...
dat_one_year_annual <- function (xraw, year = 2015) {

    x <- xraw [which (xraw$year == year), ]
...
xs <- lapply (years, function (y) dat_one_year_annual (xraw, year = y))
