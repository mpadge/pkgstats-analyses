...
nfiles_one_year <- function (x, year = 2015) {
    ...
    x1 <- x |>
        ...
        group_by (package) |>
        slice_max (date) |>
        summarise (R = 10 ^ mean (log10 (files_R [which (files_R > 0)])),
                   src = 10 ^ mean (log10 (files_src [which (files_src > 0)])),
                   inst = 10 ^ mean (log10 (files_inst [which (files_inst > 0)])),
                   vignettes = 10 ^ mean (log10 (files_vignettes [which (files_vignettes > 0)])),
                   tests = 10 ^ mean (log10 (files_tests [which (files_tests > 0)])),
                   data = 10 ^ mean (log10 (num_data_files [which (num_data_files > 0)])))
...
```{r fig3, echo = FALSE, message = FALSE, fig.width = 7, fig.height = 3.5, fig.cap = "**Figure 3**"}
p1 + p2
```
