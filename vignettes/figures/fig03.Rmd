...
f_fig3_top10 <- file.path (v_data_dir, "fig03-top10.Rds")
f_fig3_tr <- file.path (v_data_dir, "fig03-tr.Rds")
...
tr <- readRDS (f_fig3_tr)
tr_total <- tr |>
    group_by (year) |>
    summarise (n = sum (n)) |>
    mutate (translation = "ALL")
tr_total$n <- tr_total$n * max (tr$n) / max (tr_total$n)

tr_vals <- c (sort (unique (tr$translation)), "ALL")

tr <- bind_rows (tr, tr_total)
