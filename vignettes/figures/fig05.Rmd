...
```{r fig5-call-coupling-deps}
f <- file.path (v_data_dir, "fig05-coupling_deps.Rds")
if (!file.exists (f)) {
...
```{r fig5-p1-deps}
f <- file.path (v_data_dir, "fig05_deps_p1.Rds")
calc_fig5_p1_deps <- !file.exists (f)
```

```{r fig5-p1-deps-calc, eval = calc_fig5_p1_deps}
deps_p1 <- deps |>
...
```{r fig5-load-couplings-releases}
...
deps_p3 <- deps_p3 |>
    group_by (seq) |>
    summarise (packages = 10 ^ mean (log10 (afferent_npkgs [which (afferent_npkgs > 0)])),
               fns_per_pkg = 10 ^ mean (log10 (afferent_fns_per_pkg [which (afferent_fns_per_pkg > 0)]), na.rm = TRUE)) |>
    filter (seq <= 50)
