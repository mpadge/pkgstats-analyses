...
```{r fig6-external-calls-data-file}
f_fig6_exdat <- file.path (v_data_dir, "fig06-ext-calls.Rds")
calc_ext_calls_dat <- !file.exists (f_fig6_exdat)
```
...
```{r fig6-load-exdat}
x <- readRDS (f_fig6_exdat)
```
```{r fig6-deps-data-file}
f_fig6_deps <- file.path (v_data_dir, "fig06-deps.Rds")
calc_fig6_deps <- !file.exists(f_fig6_deps)
```
...
```{r fig6-coupling-data-generate, eval = !cp_dat_exists}
...
```{r fig6-dep-pkg-calls, eval = calc_top10}
x <- load_pkgstats_data (datafile, raw = TRUE, latest = FALSE) |>
...
```{r fig6-assemble, fig.width = 7, fig.height = 7, fig.cap = "**Fig. 6** Dependency networks between packages over time."}
(p1 | p2) / p3
```
