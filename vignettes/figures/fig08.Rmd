...
f <- file.path (v_data_dir, "fig08_deps_p1.Rds")
calc_fig8_p1_deps <- !file.exists (f) & !file.exists (fig08_png)
```

```{r fig08-rmcd-pkgs, eval = !file.exists (fig08_png)}
recommended <- recommended_pkgs ()
```


```{r fig8-call-coupling-deps, eval = !file.exists (fig08_png)}
f <- file.path (v_data_dir, "fig08-coupling_deps.Rds")
if (!file.exists (f)) {
...
```{r fig8-p1-deps-calc, eval = calc_fig8_p1_deps}
deps_p1 <- deps |>
...
```{r fig8-p1, eval = !file.exists (fig08_png)}
readRDS (f) |>
...
```{r fig8-load-couplings-releases, eval = !file.exists (fig08_png)}
...
```{r fig8-couplings-releases, eval = !file.exists (fig08_png)}
...
```{r fig8-afferent, eval = !file.exists (fig08_png)}
