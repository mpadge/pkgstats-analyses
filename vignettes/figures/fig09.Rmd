...
calc_graph_data <- !file.exists (graph_data_file) & !file.exists (fig09_png)
calc_graph_data_intern <- !file.exists (graph_intern_data_file) & !file.exists (fig09_png)
```
...
```{r fig9-construct, eval = !file.exists (fig09_png)}
...
coeff_fig9_p4 <- max (dat$term_edges) / max (dat$centr_dir_mn)
coeff_fig9_p4 <- max (dat$centr_dir_mn) / max (dat$term_edges)
colours_fig9_p4 <- c ("centr" = "red",
                      "vert_degree" = "green",
                      "term_edges" = "blue")
...
fig9_p4 <- dat |>
    ...
    ggplot (aes (x = year)) +
        geom_line (aes (y = centr / coeff_fig9_p4, col = "centr")) +
        ...
        scale_y_continuous (
            sec.axis = sec_axis (~.*coeff_fig9_p4, name = "Centrality")) +
...
png (fig09_png, width = 7, height = 7, units = "in", res = 300)
(fig9_p1 + fig9_p2) / (fig9_p3 + fig9_p4)
graphics.off ()
```

```{r fig9-out, echo = FALSE, out.width = 700, fig.cap = "**Figure 8** Network metrics for (A-B) the CRAN dependency network, and (C-D) Function call networks within packages."}
