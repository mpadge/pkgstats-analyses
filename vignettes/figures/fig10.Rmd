...
coeff_fig10_p2 <- max (x2$term_edges) / max (x2$centr_undir)
...
fig10_p2 <- x2 |>
    ggplot (aes (x = seq)) +
        ...
        geom_line (aes (y = vert_degree / coeff_fig10_p2, col = "vert_degree")) +
        geom_line (aes (y = term_edges / coeff_fig10_p2, col = "term_edges")) +
        scale_y_continuous (
            sec.axis = sec_axis (~.*coeff_fig10_p2, name = "Vert. degree / Term. edges")) +
...
```{r fig10-assemble, echo = FALSE, message = FALSE, fig.height = 3.5, fig.cap = "**Figure 10** Network metrics"}
fig10_p1 + fig10_p2
```
