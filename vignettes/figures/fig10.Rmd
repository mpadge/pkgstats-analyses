...
fig10_png <- file.path (here, "vignettes", "figures", "fig10.png")
```

```{r fig10-prepro, eval = !file.exists (fig10_png)}
...
```{r fig10-network-internal1, eval = !file.exists (fig10_png)}
x1 <- x |>
    select (seq, starts_with ("n_")) |>
    rename (total = n_edges, R = n_edges_r, src = n_edges_src) |>
    filter (seq <= 100)

coeff_fig10_p1 <- max (x1$n_clusters) / max (x1$total)
colours_fig10_p1 <- c ("total" = "red",
                       "R" = "green",
                       "src" = "blue",
                       "n_clusters" = "orange")
...
fig10_p1 <- x1 |>
    ggplot (aes (x = seq)) +
        ...
        geom_line (aes (y = n_clusters / coeff_fig10_p1, col = "n_clusters")) +
        #scale_colour_manual (values = colours_fig10_p1) +
        ylab ("Ratio edges/vertices") +
        scale_y_continuous (
            sec.axis = sec_axis (~.*coeff_fig10_p1, name = "Num. clusters")) +
...
```{r fig10-network-internal2, eval = !file.exists (fig10_png)}
x2 <- x |>
...
coeff_fig10_p2 <- max (x2$centr) / max (x2$term_edges)
colours_fig1e_p2 <- c ("centr" = "red",
                      "vert_degree" = "green",
                      "term_edges" = "blue")
fig10_p2 <- x2 |>
    ggplot (aes (x = seq)) +
        geom_line (aes (y = centr / coeff_fig10_p2, col = "centr")) +
        ...
        scale_y_continuous (
            sec.axis = sec_axis (~.*coeff_fig10_p2, name = "Centrality")) +
        #guides (color = guide_legend (ncol = 2)) +
        theme (legend.title = element_blank (),
...
```{r fig10-assemble, echo = FALSE, eval = !file.exists (fig10_png)}
png (fig10_png, width = 7, height = 3.5, units = "in", res = 300)
fig10_p1 + fig10_p2
graphics.off ()
```

```{r fig10-make, echo = FALSE, out.width = 700, fig.cap = "**Figure 9** Network metrics from function call networks as a function of release sequence."}
