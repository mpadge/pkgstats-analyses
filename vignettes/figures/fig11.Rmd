...
fig11_png <- file.path (here, "vignettes", "figures", "fig11.png")
calc_fig11 <- !file.exists (f_fig11) & !file.exists (fig11_png)
```
...
n2 <- data.frame (t (n2))
n2$what <- "annual"
saveRDS (rbind (n1, n2), f_fig11)
```



```{r fig11-p1-p2, eval = !file.exists (fig11_png)}

x1_cran <- readRDS (f_fig11) |>
...
p2 <- x2 |>
    ...
    geom_line (data = x2_cran, aes (y = indentation / coeff_fig11_p2, col = "indentation"), lty = 2) +
    ylab ("space (x4) | tabs") +
    scale_y_continuous (
...
```{r fig11-p3, eval = !file.exists (fig11_png)}
...
```
```{r fig11-release-intervals, eval = FALSE}
...
```{r fig11-p4, eval = !file.exists (fig11_png)}
...
```{r fig11-assemble, echo = FALSE, warning = FALSE, eval = !file.exists (fig11_png)}
png (fig11_png, width = 7, height = 7, units = "in", res = 300)
(p1 | p2) / (p3 + p4)
graphics.off ()
```

```{r fig11-out, echo = FALSE, out.width = 700, fig.cap = "**Figure 10.** Code formatting and documentation."}
knitr::include_graphics (fig11_png)
```
