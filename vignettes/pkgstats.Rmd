...
```{r tr_urls, echo = FALSE, message = FALSE, fig.cap = "**Figure 3.** (A) Annual frequencies of the top 10 translations. (B) Annual frequencies of the top 10 primary URL domains, with frequencies for GitHub divided by 20."}
x <- load_pkgstats_data (datafile, raw = TRUE, latest = FALSE) |>
    mutate (year = lubridate::year (date))
...
characteristics (Fig. 3). Unsurprisingly, R packages have always been dominated
by files in the R directory, numbers of which increased considerably up to
around 2010, after which they are generally stabilised at a mean value of
around 20 files. The `src` and `data` directories are the only places within an
R package in which numbers of files have remained relatively constant over the
preceding 20+ years. Packages generally had very few files in the `inst`
directory prior to 2010, after which they have notably increased.

The two sub-directories in which numbers of files have increased most
significantly are `vignettes` and `tests`, clearly indicating that packages
have become both better documented and better tested, especially since 2010.


```{r nfiles-plot, echo = FALSE, message = FALSE, fig.cap = "**Figure 4**"}
x <- load_pkgstats_data (datafile, raw = TRUE, latest = FALSE) |>
    mutate (year = lubridate::year (date)) |>
    group_by (year) |>
