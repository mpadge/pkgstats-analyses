...
to equivalent submission rates to CRAN (Fig. 1).
...
```
```{r plot_r_py, echo = FALSE, warning = FALSE}
...
plot_r_py (x_r, x_p,
           ...
           type = "lines") +
    ggtitle ("Figure 1: Rates of submission to pypi and CRAN",
...
Fig. 1 was due to a decrease in the rate of package updates, while rates of
submission of new packages remained fairly constant (Fig. 2).

```{r new-vs-update, echo = FALSE, warning = FALSE}
datafile <- file.path (here, "data-raw", "pkgstats-results.Rds")
plot_new_vs_update (datafile) +
    ggtitle ("Figure 2: Rates of new submissions and updates to CRAN")
```
...
over time (Fig. 3), with contributors having been rarely acknowledged at all
...
```{r num_aut_ctb, echo = FALSE, warning = FALSE}
datafile <- file.path (here, "data-raw", "pkgstats-results.Rds")
x <- load_pkgstats_data (datafile, raw = TRUE, latest = FALSE) |>
    group_by (month) |>
    summarise (n_aut = mean (desc_n_aut),
               n_ctb = mean (desc_n_ctb)) |>
    pivot_longer (cols = c (n_aut, n_ctb))
ggplot (x, aes (x = month, y = value, colour = name)) +
    geom_smooth (method = "loess", formula = y ~ x, se = FALSE) +
    ggtitle ("Figure 3: Numbers of authors and contributors") +
    theme (legend.position = c (0.1, 0.9),
           plot.title = ggplot2::element_text (hjust = 0.5),
           plot.subtitle = ggplot2::element_text (hjust = 0.5))
```

Numbers of unique licenses manifest an opposite tendency, decreasing notably
over time (Fig. 4), resulting in the values shown in Table 1.
...
ggplot (nlic, aes (x = year, y = n)) +
    geom_line () +
    ylab ("Number of distinct licenses") +
    ggtitle ("Figure 4: Numbers of licenses")
...
active acknowledgement of other languages.

```{r translations, echo = FALSE}
x <- load_pkgstats_data (datafile, raw = TRUE, latest = FALSE) |>
    mutate (year = lubridate::year (date))
...
ggplot (tr_top10, aes (x = year, y = n, color = translation)) +
    geom_line () +
    ggtitle ("Figure 5: Top 10 translations")
```
