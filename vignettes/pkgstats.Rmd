...
upon and shaped by developments in other packages or libraries. Any computer
language has associated with it an evolving ecosystem of software, and the
evolution of any one component of that ecosystem is best studied in the context
of the evolution of the ecosystem as a whole. In short, the development or
evolution of open source software is better studied in *collective* rather than
*individual* terms. 
...
which is around one tenth the size of pypi, although still amounting at the
time of writing to over 100,000 releases of around 20,000 packages. CRAN is the
dominant package distribution system for R; while other systems such as
BioConductor and R-Forge have been considered elsewhere
[@decan_development_2015], the present work focusses exclusively on CRAN.
...
"early history of the R project", itself extending ten years prior to that. In
his 2012 article, Kurt Hornik, one of the instigators of CRAN, calls for,
...
We developed [the `{pkgstats}` R package](https://docs.ropensci.org/pkgstats)
to quantify the structure and content of R packages using static code analysis
...
which can be applied to a local mirror of the CRAN archive to collate summary
statistics from all packages. The resultant data will be updated on an ongoing
basis, and published with each [latest release of the {pkgstats}
package](https://github.com/ropensci-review-tools/pkgstats/releases/latest).
...
packages](https://cran.r-project.org/src/contrib/4.2.1/Recommended/) (change
"4.2.1" to current version for updated list). All packages other than these are
referred to as "Contributed Packages". A number of the following results
distinguish between these three categories of base, recommended, and
contributed packages [@german_evolution_2013].
...
construction of "function call networks" within and between all languages. This
work generally uses the terms "functions" and "calls" to refer to more general
objects and references, because these terms more directly reflect the general
functionality of R code. The terms should be interpreted throughout as also
potentially referring to the more general concepts of "objects" and
"references", such as class inheritance mechanisms.

Default installations of "gtags" are able to parse only a restricted sub-set of
computer languages. It is possible to cross-compile "gtags" with "ctags", to
enable "gtags" to use the more expansive language parsing abilities of "ctags",
and to greatly extend the range of languages able to be parsed by "gtags". In
particular, "gtags" does not not by default parse R code, yet does so when
cross-compiled with Universal "ctags". Cross-compiled installations of "gtags"
are, however, generally several times slower than simpler installations, and so
these analyses did not use a cross-compiled version. While this meant
restricting analyses to a smaller subset of potential languages, the output of
"ctags" itself nevertheless describes tags in all possible languages, with
"gtags" only used to map function call networks in languages other than R.
Function-call networks within R were extracted from the "ctags" output as
described above, and one additional function was written in the `{pkgstats}`
package to link calls within R code as defined by the "ctags" output to
external objects defined in other languages and specified by the "gtags"
output. This enabled full function-call networks to be constructed within and
between different languages used in R packages.
...
- Lists of all imported packages used to construct dependency networks between
  packages. (Entries in other dependency fields of "Requires", "Suggests" or
  "Enhances" were not considered here.)
...
quantified as the numbers of both lines and characters in the output produced
by converting the result of `parse_Rd` to text (via the `tools::Rd2txt`
function).
...
a package depends upon for its functionality. These data can be used to
construct a full dependency network between all packages. The results below
present the following measures derived from package dependency networks, all of
which were calculated with the `{igraph}` package [@igraph].
...
and *efferent* couplings. In the context of the present analyses, for any given
package, these were defined as:
...
- *Afferent* couplings are numbers of *inbound* edges from all functions in
  other packages to functions in a given package, while
- *efferent* couplings are *outbound* edges coupling that functions in package
  with functions in all other packages.
...
This section describes statistical techniques used to summarise the large
amounts of data generated by the methods described in the preceding sections.
The large numbers of data (generally N > 105,000) ensured that all statistics
reported on in the following results were entirely significant in a
conventional frequentist-statistical terms. The results accordingly depict and
interpret qualitative trends and patterns, without implementing or reporting
values for any statistical tests. There are two further important aspects to
note in regard to statistical analyses detailed below.
...
the system as a whole, while sequential developments collectively capture the
internal development of individual packages.
...
code through such platforms, as discussed further below.
...
Proportions of packages with Uniform Resource Locator (URL) fields increased
very rapidly in the first five years of the millennium, before decreasing again
until after 2010 ("all" in legend of Fig. 1B). Most URLs declared during this
time were institutionally-hosted websites describing packages, rather than
sites hosting the actual code. One third alone were academic sites from the
English-speaking world (such as ".edu", ".ac.uk", or various other
country-specific versions of these). The increase after around 2012 in the
proportion of all packages with URLs corresponded to an increase in the
proportion of packages using git-based hosting platforms (primarily GitHub,
GitLab, and Bitbucket; "git" in legend). The general trajectory for packages
specifying URLs thus seems to have passed through an early phase during which
authors developed individual sites for each package (or group of packages), a
subsequent phase during which this individual-site approach started to decline
in popularity, and the final and currently increasing phase of package code
being directly hosted on code-sharing platforms.

```{r pre-2010-urls, echo = FALSE, eval = FALSE}
here <- here::here ()
datafile <- file.path (here, "data-raw", "pkgstats-results.Rds")
x <- load_pkgstats_data (datafile, raw = TRUE, latest = FALSE)
x2010 <- x [x$year < 2010, ]
u <- regmatches (x2010$urls, regexpr ("http(s?)\\:\\/\\/[^\\/]*\\/", x2010$urls))
utab <- sort (table (u), decreasing = TRUE)
utab <- data.frame (
    url = names (utab),
    n = as.integer (utab),
    proportion = as.double (utab) / length (u)
)
```


```{r fig01, child = "figures/_fig01.Rmd", eval = TRUE, echo = FALSE}
```

Figure 1C shows a roughly linear increase in license declarations up to around
2007, followed by a pronounced drop and progression into a roughly stable range
of somewhere over 50 distinct entries. This high values reflects the fact that
numbers of unique licenses primarily reflect numbers of ways of declaring
licenses, rather than numbers of actual licenses (for example, "GPL >= 2.0" is
different to "Any GPL license version 2 or higher", although both clearly refer
to the same license). The different ways of specifying the three versions of
GPL licenses have accounted for over 83% of all CRAN packages, with the second
most common type license being MIT, at 9.6%, followed by BSD at 2.0%. One of
these three classes of licenses are used in 95% of all CRAN packages.
...
```{r fig02, child = "figures/_fig02.Rmd", eval = TRUE, echo = FALSE}
```

Finally, Fig. 2B extends from the results of Fig. 1B to show temporal evolution
of the top ten URLs specified in CRAN packages, clearly revealing a sharp
increase after around 2010 following the advent of code hosting platforms.
GitHub has always dominated, while r-forge has declined progressively since
around 2015. The most notable increase has been the emergence of rOpenSci as
provider of the only URL to come close to within 1/20th of the popularity of
GitHub over the past few years.
...
indicating that packages have become progressively better tested. This recent,
generally linear increase started a couple of years after the initial release
of what has become the most widely-used package for structuring tests,
`{testthat}` [@testthat].


```{r fig03, child = "figures/_fig03.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```

Figure 3B shows equivalent changes within individual packages across successive
releases. Numbers of files have generally progressively increased as packages
have matured, with especially pronounced increases in the `/R` and `/inst`
directories, the latter somewhat offset after around 15 releases by decreases
in the `src` directory. These changes in `/src` and `/inst` directories in the
...
packages. Numbers of test files generally increase out to around 15 releases,
after which they generally stabilise. The only directory in which numbers of
files do not notably increase as packages mature is `/vignettes`.
...
```{r fig04, child = "figures/_fig04.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```
...
```{r fig05, child = "figures/_fig05.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```
...
```{r fig06, child = "figures/_fig06.Rmd", eval = TRUE, echo = FALSE}
```
...
```{r fig07, child = "figures/_fig07.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```
...
have broadly stabilised over the preceding five or so years, following
generally abrupt changes between around 2010 to 2015. The proportion of
terminal vertices, or packages not imported by any other packages, has recently
remained broadly stable at around 55-60% of all packages. The mean vertex
degree of the dependency network has nevertheless progressively increased,
especially since 2015, indicating that those packages which are imported by
others have been imported by increasing numbers of other packages (Fig. 8A).

```{r fig08, child = "figures/_fig08.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```

Such an increase in average numbers of imports has nevertheless had little
apparent effect on measures of network centrality (Fig. 8B), with both measures
remaining broadly stable since 2015. The increasing vertex degree of Fig. 8A
nevertheless seems reflected in decreasing mean distance between each pair of
packages in Fig. 8B. More broadly, since numbers of terminal vertices have
remained generally stable, while centrality has decreased, the initial increase
in mean distance likely reflects terminal vertices becoming further away, for
example through the network evolving a number of distinct centres. The more
recent decrease in mean distance may then reflect an equivalent centralisation
...
```{r fig09, child = "figures/_fig09.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```
...
[@roxygen2] which has indubitably done more than any other package to
standardise documentation of R functions. This increase in `/R` documentation
is likely a direct reflection of the widespread adoption of `{roxygen2}` as the
documentation standard for R packages.

```{r fig10, child = "figures/_fig10.Rmd", eval = TRUE, echo = FALSE, warning = FALSE}
```
...
they might otherwise have reached from authors' own GitHub profile pages. Thus
the increases in both GitHub and rOpenSci URLs reflect increased community
...
The results presented here contrasted and compared two distinct ways of
quantifying "annual" statistics. The "snapshot" values considered the
collective properties of the system as it existed in each year, including
latest versions of all packages regardless of when those packages were last
updated. In contrast, "annual" values only considered software actively
uploaded in each year. As stated at the outset, snapshot values may be
...
led to opposing trajectories of increasing Gini coefficients.

While detailed consideration of these differences lies beyond the scope of the
present work, it is nevertheless important to note that for discrete,
non-negative variables such as counts, Gini coefficients must generally
increase with the scale or maximal values of those variables. Thus if average
numbers of imported packages (or functions) increases over time, so should Gini
coefficients. This consideration alone suggests that observations of
*increasing* Gini coefficients should be interpreted cautiously, as they may
merely reflect uniformly increasing dependence on external packages, while
observations of *decreasing* Gini coefficients, as with our "snapshot" values,
may reflect genuine changes. Whatever the underlying cause of these observed
...
packages. Collective properties alone are inadequate to understand how a system
has actually evolved. Moreover, future work will likely have to consider and
contrast systems for weighting the relative contributions of different packages
to the software ecosystem as a whole. The present results applied uniform
weights to all packages, regardless of whether an individual package has 100
lines of code and has never been updated or even downloaded, or whether it has
thousands of lines of code and orders of magnitude more downloads. Deriving and
applying weighting schemes will be an extremely difficult endeavour, especially
because there can never be any single, optimal scheme for doing so, and
reported results will be intimately dependent on any chosen weighting scheme.
We hope that the present work provides a firm empirical basis from which to
begin the task of developing such schemes in order to further understanding of
...
extremely valuable resource. We hope that this present study represents only a
beginning of subsequent attempts to delve deeper in to the wealth of data
provided by CRAN.
