...
logmean <- function (x, ...) {
    10 ^ mean (log10 (x [which (x > 0 & !is.nan (x) & is.finite (x))]), na.rm = TRUE)
}
```
...
The size of pypi necessarily restricted analyses
...
<!-- TODO: summarise results/trends from these and the preceding pypi analysis -->

CRAN, the dominant repository of R packages, is more modest in size and more
...
analysis ran on a local mirror of CRAN as of July 2022, comprising <XXXXEXACT>
...
lic <- data.frame (license = names (lic), n = as.integer (lic))
...
years to stabilize from 2005-2020. 
The slight decreases discernible
...
Total numbers of R functions per package initially increased until around 2007, peaking at
...
x_rel <- x |>
    group_by (seq) |>
    filter (seq \<= 100) |>
    summarise ( n = logmean (n_fns_r_exported + n_fns_r_not_exported +
                             n_fns_src, na.rm = TRUE))
...
x_rel$fit <- fit x_rel <- pivot_longer (x_rel, c (n, fit))
